...
let query3 = 'from(bucket: "MetaData")\
                |> range(start: -30d)\
...
app.post('/register-user', (req, res) => {
    ...
    if(!name.length || !password.length){
      ...
      const point = new Point("LoginIds").tag("Credentials","UsernamePassword").stringField("Credentials","["+name+":"+password+"]");
...
app.post('/login-user', (req, res) => {
    ...
    queryApi.queryRows(query3, {
        ...
        complete() {
            ...
            if (UserPass.includes("["+username.toLowerCase()+":"+password.toLowerCase()+"]")){
              console.log("valid");
              // window.navigate("Noice")
              // res.redirect('/Noicedata');
              openwin("http://www." + req.headers['host'] + '/NoiceData.html');
              console.log("http://www." + req.headers['host'] + '/NoiceData.html');
